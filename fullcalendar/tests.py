...
class ConvertFieldNames(TestCase):
    def test_conversion(self):
        ...
        self.assertEqual(convert_field_names(l),
                         [{'start': '2013-11-27', 'end': '2013-11-29', 'allDay': 'true',
                           '__size__': 1, '__toString__': 'false'}])


class CalendarOptions(TestCase):
    def test_options_string(self):
        s = '{timeFormat: "H:mm", header: {left: "prev,next today", center: "title", ' \
            'right: "month,agendaWeek,agendaDay",}'
        self.assertEqual(calendar_options('all_events/', s),
                         '{events: "all_events/", timeFormat: "H:mm", header: {left: "prev,next today", '
                         'center: "title", right: "month,agendaWeek,agendaDay",}')

    def test_options_string_with_whitespaces(self):
        s = '   {timeFormat: "H:mm", header: {left: "prev,next today", ' \
            'center: "title", right: "month,agendaWeek,agendaDay",}    '
        self.assertEqual(calendar_options('all_events/', s),
                         '{events: "all_events/", timeFormat: "H:mm", header: {left: "prev,next today", '
                         'center: "title", right: "month,agendaWeek,agendaDay",}')
